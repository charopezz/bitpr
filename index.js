...
const { rp_menu } = require('./src/rp_menu')
const { me registrar } require('./src/me registrar')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
        ...
        case '(cudorma)':
					...
					buffer = await getBuffer(`(https://images.app.goo.gl/YC9CdgCpxFmqydwp9)`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(oia que cudorna bunita )'})
					break
					case 'nsfwbobs': 
						try {
							if (!isNsfw) return reply('❌ *NSFW Desativado* ❌')
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
       ...
       case 'rp_menu':
		...
		client.sendMessage(from, rp_menu(prefix, sender), text, {quoted: mek})
                break
				case 'ytmp3':
               ...
               if (data.error) return reply(data.error)
				...
				case 'blocklist':
                   ...
                   case 'chatlist':
					...
					break
      case 'me registrar ':
		client.sendMessage(from, me registrar(prefix, sender), text, {quoted: mek})
                break
				case 'animecry':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
