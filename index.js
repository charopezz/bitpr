	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve meu amigo @${num.split('@')[0]}\n𝗢𝗟𝗔,𝘀𝗼𝘂 𝘂𝗺 𝗯𝗼𝘁 𝗱𝗲 𝗮𝗷𝘂𝗱𝗮 𝘀𝗲𝗷𝗮 𝗯𝗲𝗺 vindor ao 𝘀𝗲𝗿𝘃𝗶𝗱𝗼𝗿 𝗲𝘀𝗽𝗲𝗿𝗼 𝗾𝘂𝗲 𝗴𝗼𝘀𝘁𝗲 𝗱𝗲 𝘂𝗺 !𝗿𝗽 *${mdata.subject}*`
				...
				teks = `mais um saiu😔 por que isso!!!! @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
